...
/**
 * The index file for the voucherseries route.
...
var flow  = require('./voucherseries.flow');
...
router.get('/cleanAndFetch', function (req, res) {
    flow.cleanAndFetch()
        .then(function (_res) {
            res.status(200).send('Database cleaned and all voucherseries fetched.');
        })
        .catch(handleError);
});
...
router.get('/fetchNewlyModified', function (req, res) {
    flow.fetchNewlyModified()
        .then(function (_res) {
            res.status(200).send('Newly modifified voucherseries fetched.');
        })
        .catch(handleError);
});
 ...
 * A response with the statuscode 200 containing the voucherseries are returned.
 */
...
/**
 * Gets all voucherseries from the db, includes historical data.
 * A response with the statuscode 200 containing the voucherseries are returned.
 */
