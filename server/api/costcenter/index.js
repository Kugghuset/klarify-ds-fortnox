...
/**
 * The index file for the costcenter route.
...
var flow  = require('./costcenter.flow');
...
router.get('/cleanAndFetch', function (req, res) {
    flow.cleanAndFetch()
        .then(function (_res) {
            res.status(200).send('Database cleaned and all costcenters fetched.');
        })
        .catch(handleError);
});
...
router.get('/fetchNewlyModified', function (req, res) {
    flow.fetchNewlyModified()
        .then(function (_res) {
            res.status(200).send('Newly modifified costcenters fetched.');
        })
        .catch(handleError);
});
 ...
 * A response with the statuscode 200 containing the costcenters are returned.
 */
...
/**
 * Gets all costcenters from the db, includes historical data.
 * A response with the statuscode 200 containing the costcenters are returned.
 */
