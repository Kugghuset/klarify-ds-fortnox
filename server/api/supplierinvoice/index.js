...
var flow  = require('./supplierinvoice.flow.js');
...
router.get('/cleanAndFetch', function (req, res) {
    flow.cleanAndFetch()
        .then(function (_res) {
            res.status(200).send('Database cleaned and all supplierinvoices fetched.');
        })
        .catch(handleError);
});
...
router.get('/fetchNewlyModified', function (req, res) {
    flow.fetchNewlyModified()
        .then(function (_res) {
            res.status(200).send('Newly modifified supplierinvoices fetched.');
        })
        .catch(handleError);
});
 ...
 * A response with the statuscode 200 containing the supplierinvoices are returned.
 */
...
/**
 * Gets all supplierinvoices from the db, includes historical data.
 * A response with the statuscode 200 containing the supplierinvoices are returned.
 */
